...
def log_health():
    ...
    if height is not None and (height <= 0 or height > 300):
        return jsonify({"error": "Invalid height."}), 400

    if blood_sugar is not None:
        try:
            blood_sugar = int(blood_sugar)
            if blood_sugar <= 0 or blood_sugar > 1000:
                return jsonify({"error": "Invalid blood sugar."}), 400
        except:
            return jsonify({"error": "Blood sugar must be a number."}), 400
...
def get_records():
    user = get_current_user()
    records = HealthRecord.query.filter_by(user_id=user.id)\
...
def health_summary():
    ...
    summary = {
        ...
        "weight_trend": [
            ...
            for r in reversed(records) if r.weight
        ],
    }
